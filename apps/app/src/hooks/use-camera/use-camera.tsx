...
import { useCameraDr } from "./use-camera.dr"

export default function useCamera({ dictionary }: { dictionary: TDictionary<typeof useCameraDr> }) {
  ...
  const takePhoto = useCallback(
    async (opts?: { openGalery?: (() => void) | null }): Promise<File> => {
      ...
      if (!contentRef.current) {
        throw new Error("Please render the `useCamera` content")
      }
        ...
        >
          <ModalContent className="flex flex-col">
            ...
            <div className="relative flex-1 overflow-hidden">
              <video ref={videoRef} className="relative z-10 mx-auto h-full object-contain" />
              <video
                ref={videoBgRef}
