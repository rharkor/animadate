...
else ifeq ($(CMD),app/wss)
	docker build -f apps/app/docker/wss.Dockerfile -t animadate/app-wss --network host .
	docker run -e PORT=8080 --network host animadate/app-wss
else ifeq ($(CMD),app/app)
	docker build -f apps/app/docker/app.Dockerfile -t animadate/app-app --network host .
	docker run -e PORT=8080 --network host animadate/app-app
